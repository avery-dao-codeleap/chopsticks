...
export async function getPendingParticipants(requestId: string): Promise<{
  ...
  try {
    ...
    // Map users from array to single object (Supabase FK join quirk)
...
/**
 * Leave a meal request (for joined participants, not the creator)
 * Removes from request_participants and chat_participants
 */
export async function leaveRequest(requestId: string): Promise<{
  error: Error | null;
}> {
  try {
    const { data: { user } } = await supabase.auth.getUser();
    ...
    // Remove from request_participants
    const { error } = await supabase
      .from('request_participants')
      .delete()
      .eq('request_id', requestId)
    ...
    // Also remove from chat_participants if a chat exists
