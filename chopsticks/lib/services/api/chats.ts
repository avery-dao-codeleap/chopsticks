  ...
  try {
    ...
    const chatsWithDetails = await Promise.all(
      (data || []).map(async (item) => {
        ...
        // Extract first element from arrays (Supabase returns arrays for foreign key relations)
        const rawMealRequests = chat.meal_requests;
        const mealRequest = Array.isArray(rawMealRequests) ? rawMealRequests[0] : rawMealRequests;

        // Fetch restaurant separately to avoid RLS issues with nested queries
        let restaurant = null;
        if (mealRequest?.restaurant_id) {
          const { data: restaurantData } = await supabase
            .from('restaurants')
            .select('id, name, address, district')
            .eq('id', mealRequest.restaurant_id)
            .single();
...
export async function getChatMessages(chatId: string) {
  try {
    ...
    // Map messages to extract user data from arrays
...
export async function getChat(chatId: string) {
  try {
    ...
    // Extract meal_requests from array
    const mealRequest = Array.isArray(data.meal_requests) ? data.meal_requests[0] : data.meal_requests;

    // Fetch restaurant separately to avoid RLS issues
    let restaurant = null;
    if (mealRequest?.restaurant_id) {
      const { data: restaurantData } = await supabase
        .from('restaurants')
    ...
    // Map participants to extract user data from arrays
    ...
    return {
      data: {
        ...data,
        meal_requests: mealRequest ? { ...mealRequest, restaurants: restaurant } : mealRequest,
...
export async function removeUserFromChat(chatId: string, userId: string) {
  try {
    ...
    const mealReq = Array.isArray(chat?.meal_requests) ? chat.meal_requests[0] : chat?.meal_requests;
    if (!chat || !mealReq || mealReq.requester_id !== user.user.id) {
