...
import {
  ...
  removeUserFromChat,
  leaveChat,
} from '@/lib/services/api/chats';

const CHAT_LAST_SEEN_KEY = 'chat_tab_last_seen';

/**
 * Mark chat tab as seen (call when user visits chat tab)
 */
export async function markChatTabSeen() {
  await AsyncStorage.setItem(CHAT_LAST_SEEN_KEY, new Date().toISOString());
}

/**
 * Lightweight hook to check if there are unread chat messages.
 * Returns true if any chat has messages from others since last visit.
 */
export function useUnreadDot() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const lastSeen = await AsyncStorage.getItem(CHAT_LAST_SEEN_KEY);

      // Get chat IDs where user participates
      const { data: chatParticipants } = await supabase
        .from('chat_participants')
        .select('chat_id')
        .eq('user_id', user.id);

      if (!chatParticipants?.length) return false;

      const chatIds = chatParticipants.map(p => p.chat_id);

      // Check for messages from others since last seen
      let query = supabase
        .from('messages')
